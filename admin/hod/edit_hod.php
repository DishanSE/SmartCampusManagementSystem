...
// Get lecturer data
$stmt = $conn->prepare("SELECT email, first_name, last_name FROM users WHERE user_id = ? AND role = 'hod'");
...
$hods = $result->fetch_assoc();
$email = $hods['email'];
$firstName = $hods['first_name'];
$lastName = $hods['last_name'];
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty(trim($_POST["email"]))) {
        ...
        if ($email != trim($_POST["email"])) {
            ...
            if ($stmt = $conn->prepare($sql)) {
                ...
                $stmt->bind_param("si", $param_email, $hodId);
    ...
    if (empty($email_err) && empty($firstName_err) && empty($lastName_err) && empty($password_err)) {
        ...
        if (empty($password)) {
            ...
            $stmt->bind_param("sssi", $email, $firstName, $lastName, $hodId);
        } else {
            // Update including password
            $sql = "UPDATE users SET email = ?, password = ?, first_name = ?, last_name = ? WHERE user_id = ?";
            $stmt = $conn->prepare($sql);
            $hashed_password = password_hash($password, PASSWORD_DEFAULT);
            $stmt->bind_param("ssssi", $email, $hashed_password, $firstName, $lastName, $hodId);
        }
    ...
    <div class="dashboard-container">
        ...
        <div class="main-content">
            <div class="header">
                ...
                <a href="hod.php" class="btn">Back to HOds</a>
            </div>
            ...
            <div class="form-container">
                ...
                <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]) . '?id=' . $hodId; ?>" method="post">

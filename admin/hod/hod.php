...
if (isset($_GET['action']) && $_GET['action'] == 'delete' && isset($_GET['id'])) {
    ...
    if ($hodId == $_SESSION['user_id']) {
        $deleteError = "You cannot delete your own account.";
    } else {
        // Check if lecturer exists
        $checkStmt = $conn->prepare("SELECT user_id FROM users WHERE user_id = ? AND role = 'hod'");
        $checkStmt->bind_param("i", $hodId);
        $checkStmt->execute();
        ...
        if ($checkResult->num_rows == 1) {
            ...
            $deleteStmt->bind_param("i", $hodId);
            
            if ($deleteStmt->execute()) {
                $deleteSuccess = "HOD deleted successfully.";
            } else {
                $deleteError = "Error deleting HOD. Please try again.";
            }
            
            $deleteStmt->close();
        } else {
            $deleteError = "HOD not found.";
        }
...
$countQuery = "SELECT COUNT(*) as total FROM users WHERE role = 'hod'";
...
$query = "SELECT user_id, email, first_name, last_name, created_at FROM users WHERE role = 'hod' ORDER BY created_at DESC LIMIT ? OFFSET ?";
    ...
    <div class="dashboard-container">
        ...
        <div class="main-content">
            <div class="header">
                ...
                <a href="add_hod.php" class="btn">Add New HOD</a>
            </div>
            ...
            <div class="table-container">
                <table>
                    ...
                    <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="action-buttons">
                                            <a href="edit_hod.php?id=<?php echo $lecturer['user_id']; ?>" class="btn btn-sm">Edit</a>
                                            <a href="view_hod.php?id=<?php echo $lecturer['user_id']; ?>" class="btn btn-sm" style="background-color: #2ecc71;">View</a>
                                            <a href="hod.php?action=delete&id=<?php echo $lecturer['user_id']; ?>" class="btn btn-sm btn-danger delete-btn" onclick="return confirm('Are you sure you want to delete this HOD?');">Delete</a>
                                        </div>
                                    </td>
                                </tr>
                            <?php endwhile; ?>
                        <?php else: ?>
                            <tr>
                                <td colspan="5" style="text-align: center;">No HOD found.</td>
                            </tr>
                        <?php endif; ?>
                    </tbody>
                </table>

...
// Get lecturer data
$stmt = $conn->prepare("SELECT email, first_name, last_name FROM users WHERE user_id = ? AND role = 'lecturer'");
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty(trim($_POST["email"]))) {
        ...
        if ($email != trim($_POST["email"])) {
            ...
            if ($stmt = $conn->prepare($sql)) {
                ...
                $stmt->bind_param("si", $param_email, $lecturerId);
    ...
    if (empty($email_err) && empty($firstName_err) && empty($lastName_err) && empty($password_err)) {
        ...
        if (empty($password)) {
            ...
            $stmt->bind_param("sssi", $email, $firstName, $lastName, $lecturerId);
        } else {
            // Update including password
            $sql = "UPDATE users SET email = ?, password = ?, first_name = ?, last_name = ? WHERE user_id = ?";
            $stmt = $conn->prepare($sql);
            $hashed_password = password_hash($password, PASSWORD_DEFAULT);
            $stmt->bind_param("ssssi", $email, $hashed_password, $firstName, $lastName, $lecturerId);
        }
    ...
    <div class="dashboard-container">
        ...
        <div class="sidebar">
            ...
            <ul class="sidebar-menu">
                ...
                <li><a href="lecturers.php" class="active">Manage Lecturer</a></li>
        ...
        <div class="main-content">
            ...
            <div class="form-container">
                ...
                <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]) . '?id=' . $lecturerId; ?>" method="post">
